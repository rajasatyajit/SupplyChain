...
func TestDatabaseAndPostgresStore_Integration(t *testing.T) {
	...
	// Health should pass
	if err := db.Health(ctx); err != nil { t.Fatalf("db health: %v", err) }

	// Apply migrations
	pool := dbpoolFromDB(db)
	b, err := os.ReadFile(pgMigrationsPath(t))
	if err != nil { t.Fatalf("read migrations: %v", err) }
	if _, err := pool.Exec(ctx, string(b)); err != nil { t.Fatalf("apply migrations: %v", err) }

	// Exec
	if err := db.Exec(ctx, "SELECT 1"); err != nil { t.Fatalf("exec: %v", err) }
	// Query
	if _, err := db.Query(ctx, "SELECT 1"); err != nil { t.Fatalf("query: %v", err) }
	// QueryRow
	if r := db.QueryRow(ctx, "SELECT 1"); r == nil { t.Fatalf("expected non-nil row") }
	...
	if err := st.UpsertAlerts(ctx, alerts); err != nil { t.Fatalf("upsert: %v", err) }

	got, err := st.GetAlert(ctx, "int-1")
	if err != nil || got == nil { t.Fatalf("get alert: %v, %+v", err, got) }

	list, err := st.QueryAlerts(ctx, models.AlertQuery{Sources: []string{"itest"}, Limit: 10})
	if err != nil { t.Fatalf("query alerts: %v", err) }
	if len(list) == 0 { t.Fatalf("expected at least one alert") }
}
