...
func TestPostgresStore_QueryAlerts_ErrorFromDB(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "query alerts") {
		t.Errorf("wrap missing: %v", err)
	}
}
...
func TestPostgresStore_QueryAlerts_InvalidRowsType(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "invalid rows type") {
		t.Errorf("got %v", err)
	}
}
...
func TestPostgresStore_GetAlert_InvalidRowType(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "invalid row type") {
		t.Errorf("got %v", err)
	}
}
